print("loading NullRDC rime-uc dissector")
...
function rime_proto.dissector(buffer,pinfo,tree)
	...
	local rime_tree = tree:add(rime_proto, buffer(9, 6), "RIME")
	rime_tree:add(buffer(9,2), "Channel: " .. buffer(9,2):le_uint())
	rime_tree:add(buffer(11,2),"Source Address: " .. buffer(13,1):uint() .. ":" .. buffer(14,1):uint())
	rime_tree:add(buffer(13,2),"Destination Address: " .. buffer(11,1):uint() .. ":" .. buffer(12,1):uint())

	local data_tree = tree:add(rime_proto, buffer(13, buffer:len() - 15), "Payload")

	pinfo.cols.info = "Rime uc on channel " .. buffer(9,2):le_uint()
	pinfo.cols.dst = "" .. buffer(11,1):uint() .. ":" .. buffer(12,1):uint()
