print("loading ContikiMAC rime-ibc dissector")
...
function rime_proto.dissector(buffer,pinfo,tree)
	...
	local rime_tree = tree:add(rime_proto, buffer(11, 4), "RIME")
	rime_tree:add(buffer(11,2), "Channel: " .. buffer(11,2):le_uint())
	rime_tree:add(buffer(13,2),"Source Address: " .. buffer(13,1):uint() .. ":" .. buffer(14,1):uint())

	local data_tree = tree:add(rime_proto, buffer(15, buffer:len() - 17), "Payload")

	pinfo.cols.info = "Rime ibc on channel " .. buffer(11,2):le_uint()
	pinfo.cols.src = "" .. buffer(13,1):uint() .. ":" .. buffer(14,1):uint()
