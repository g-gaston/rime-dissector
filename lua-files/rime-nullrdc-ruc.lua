print("loading NullRDC rime-ruc dissector")
...
function rime_proto.dissector(buffer,pinfo,tree)
	...
	local rime_tree = tree:add(rime_proto, buffer(9, 7), "RIME")
	rime_tree:add(buffer(9,2), "Channel: " .. buffer(9,2):le_uint())
	rime_tree:add(buffer(11,1), "Packet type: " .. buffer(11,1):bitfield(0,1))
	rime_tree:add(buffer(11,1), "Packet ID: " .. buffer(11,1):bitfield(1,2))
	rime_tree:add(buffer(11,3),"Destination Address: " .. buffer(11,2):bitfield(3,8) .. ":" .. buffer(12,2):bitfield(3,8))
	rime_tree:add(buffer(13,3),"Source Address: " .. buffer(13,2):bitfield(3,8) .. ":" .. buffer(14,2):bitfield(3,8))

	local data_tree = tree:add(rime_proto, buffer(16, buffer:len() - 18), "Payload")

	pinfo.cols.info = "Rime ruc on channel " .. buffer(9,2):le_uint()
	pinfo.cols.src = "" .. buffer(13,2):bitfield(3,8) .. ":" .. buffer(14,2):bitfield(3,8)
	pinfo.cols.dst = "" .. buffer(11,2):bitfield(3,8) .. ":" .. buffer(12,2):bitfield(3,8)
end

-- get wiretap table
table = DissectorTable.get("wtap_encap")
-- and add rime protocol
table:add(wtap["IEEE802_13_4"], rime_proto)
